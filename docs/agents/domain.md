...
If this repo ever splits into genuinely separate bounded contexts, the escape hatch is a root `CONTEXT-MAP.md` pointing at one `CONTEXT.md` per context, with context-scoped ADRs alongside each.
Until then, treat the root pair above as the only domain docs.
